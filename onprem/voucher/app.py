...
with open('/fence/keys/jwt_public_key.pem', "r") as f:
    public_key = f.read()

with open('/fence/keys/jwt_private_key.pem', "r") as f:
    private_key = f.read()
...
def extract_access_token(r):
    ...
    if 'Authentication' in request.headers:
        ...
        app.logger.debug('checking for access_token in cookies')
        access_token = r.cookies.get('access_token')
    if not access_token:
        app.logger.debug('No access token found in cookie or header')
...
def nginx_auth():
    ...
    app.logger.debug({'user_name': user_name, 'roles': roles})
